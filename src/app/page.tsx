    ...
    <main className="min-h-screen bg-cream">
      ...
      <section className="py-20 container mx-auto px-4">
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
          ...
          <button className="text-primary font-bold hover:underline decoration-accent decoration-2 underline-offset-4">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {MOCK_PRODUCTS.map((product) => (
            ...
            >
              ...
              <p className="text-xs font-bold text-accent uppercase tracking-widest mb-1">{product.artisan.name}</p>
      ...
      <section className="py-24 bg-white overflow-hidden">
        <div className="container mx-auto px-4 mb-12 flex justify-between items-end">
          ...
          <Link href="/artisans/elena-ross" className="hidden md:flex items-center gap-2 text-primary font-bold hover:text-accent transition-colors group">
            View All Studios
        ...
        <div className="relative">
          <div className="flex gap-8 px-4 overflow-x-auto pb-8 snap-x no-scrollbar">
            {Array.from(new Set(MOCK_PRODUCTS.map(p => p.artisan.name))).map((name, idx) => {
              const artisan = MOCK_PRODUCTS.find(p => p.artisan.name === name)!.artisan;
              return (
                <motion.div 
                  key={name}
                ...
                >
                  <Link href={`/artisans/${name.toLowerCase().replace(/ /g, "-")}`} className="group block h-full">
                    <div className="bg-cream rounded-[3rem] p-8 border border-primary/5 shadow-xl shadow-primary/5 transition-all hover:shadow-2xl hover:shadow-primary/10 hover:-translate-y-2 h-full flex flex-col">
                      <div className="relative w-24 h-24 mb-6">
                        ...
                        <div className="relative w-full h-full rounded-full overflow-hidden border-4 border-white shadow-lg">
                          <Image src={artisan.avatar} alt={name} fill className="object-cover" />
                        </div>
                      </div>
                      
                      <h3 className="text-2xl font-heading font-bold text-primary mb-1">{name}</h3>
